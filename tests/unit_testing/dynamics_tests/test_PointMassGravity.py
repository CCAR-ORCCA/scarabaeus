...
# region Tests #
#--------------#
def test_initialization(state,origin):
    """
         Verifies that object is constructed correctly.
    """
    # intialize
    point_mass_gravity = scb.PointMassGravity(state,J2000)
    
    # ensure correct properties
    assert point_mass_gravity._origin_name       == origin.name
    assert point_mass_gravity._origin_mu         == awu.get_value_in_target_units(origin.grav_param, km**3/sec**2)
    assert point_mass_gravity._ref_frame         == J2000
